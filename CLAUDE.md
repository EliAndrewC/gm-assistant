...
- **Stop-work ritual - run EVERY time you stop working** (reporting a task done, reaching a milestone, or pausing for GM input). **SCRIPTED (GM 2026-07-21): run [`scripts/ritual.sh`](scripts/ritual.sh) from inside the clone instead of hand-typing the steps** - `ritual.sh sync-in` (start of work), `ritual.sh push` (commit first yourself; locked pull+push, exit 3 = incoming changes overlapped your files, rerun the gate and fix forward), `ritual.sh render-sync [--regen]` (tip-guarded, locked, checksum-verified; `--regen` runs every gen from its own directory, killing the Mode A cwd trap), `ritual.sh done` (push + render-sync). Hand-typed rituals caused every sync incident on record (skipped flock, wrong-cwd rsync, error-suppressed cp, stray Mode A outputs); the numbered steps below are the SPEC the script implements - and the fallback if it misbehaves:
