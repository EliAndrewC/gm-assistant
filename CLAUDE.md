...
- **`.clones/` is gitignored and lives on the host mount** (`/gm-assistant` is bind-mounted), so clones survive container rebuilds. If `.clones/<session-name>` already exists (a resumed session), reuse it: run the sync-in step below (`git pull origin main` inside the clone) before resuming work.
...
- **Stop-work ritual - run EVERY time you stop working** (reporting a task done, reaching a milestone, or pausing for GM input):
  ...
  2. **Sync in:** from inside the clone, `git pull origin main` and resolve any merge conflicts there in the clone. If the merge brought in someone else's changes to files you touched, rerun the relevant gate (`make done` for webapp work, the skill's `pytest --cov` for skill work) and fix anything that broke before proceeding.
  3. **Push back to main:** from inside the clone, `git push origin main` - a fast-forward, since step 2 merged main's tip. Main has `receive.denyCurrentBranch = updateInstead` set (one-time setup: `git -C /gm-assistant config receive.denyCurrentBranch updateInstead`; redo after any fresh checkout of this repo), so the push also updates main's working tree.
  4. **If the push is refused** (updateInstead refuses when main has uncommitted changes to tracked files), fall back to `git -C /gm-assistant pull --ff-only .clones/<session-name> main`; if that is also refused, stop and tell the GM main needs a commit or stash - do not force anything.
