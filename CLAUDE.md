...
- **Clone name = the Claude Code session name, kebab-cased**: lowercase, spaces to hyphens, punctuation dropped. "GM assistant" works in `.clones/gm-assistant`; "Diagram (city)" works in `.clones/diagram-city`. If the session has no name, pick a short descriptive kebab-case name and tell the GM which one you used.
...
- **Create the clone (cheap):** `git clone /gm-assistant .clones/<session-name>` - a local clone hardlinks the object store, so it is near-instant and does not double the disk. The clone's `origin` is the main `/gm-assistant` checkout, which makes the sync commands below symmetric (`pull origin` / `push origin`).
