...
- **Iterate on the motivating artifact; sweep the pool exactly once, at the end.** The red/green loop runs against the one map (or fixture) that exhibits the defect - single-map regen + gate is ~1-7s, so cycles are near-free. The full-pool run (`make done` regenerates every map) is reserved for AFTER the motivating artifact is in a good state - but it is MANDATORY then whenever shared engine code changed (`settlement.py`, `check_village.py`, `waterfields.py`): every map is a downstream artifact of the engine, and the sweep is what turns "no other map has this bug" from a hope into a verified claim. Anti-pattern (this bit the scale-bar feature, pre-speedup): using the full suite as the FIRST verification of an engine change - a failure that would surface in ~6s on one map surfaced 17 minutes in.
...
- **Clone name = the Claude Code session name, kebab-cased**: lowercase, spaces to hyphens, punctuation dropped. "GM assistant" works in `.clones/gm-assistant`; "Diagram (city)" works in `.clones/diagram-city`. **Only names the GM chose count** - a name set via `/rename` or `claude -n` (or requested in chat) is the session name; an auto-generated/derived title is NOT (check `~/.claude/sessions/<pid>.json`: `nameSource: "derived"` means auto-generated, fair game; a GM-set name must never be overridden). If the GM has not named the session, the default name is **"gm assistant"** → `.clones/gm-assistant`. Claude cannot rename a session from inside it (`/rename` is user-facing only, verified 2026-07-20) - just use the default clone name and note it; the GM can `/rename gm assistant` or launch with `claude -n "gm assistant"` if they want the display name to match. Caveat: two unnamed sessions running in parallel would share the default clone, so when deliberately running parallel work the GM should `/rename` each session distinctly.
...
- **Stop-work ritual - run EVERY time you stop working** (reporting a task done, reaching a milestone, or pausing for GM input). **SCRIPTED (GM 2026-07-21): run [`scripts/sync-with-main.sh`](scripts/sync-with-main.sh) from inside the clone instead of hand-typing the steps** - `sync-with-main.sh sync-in` (start of work), `sync-with-main.sh push` (commit first yourself; locked pull+push, exit 3 = incoming changes overlapped your files, rerun the gate and fix forward), `sync-with-main.sh render-sync` (always regenerates - every gen from its own directory, killing the Mode A cwd trap - then tip-guarded, locked, checksum-verified copy), `sync-with-main.sh done` (push + render-sync). Hand-typed rituals caused every sync incident on record (skipped flock, wrong-cwd rsync, error-suppressed cp, stray Mode A outputs); the numbered steps below are the SPEC the script implements - and the fallback if it misbehaves:
  ...
  5. **Render sync into main (only when your commits touched diagram pool sources or the engine).** Main's working tree is where the GM browses EVERY pool render in one place, so the gitignored derived renders (Mode B `.svg`+`.png` in `pool/{hamlets,villages,towns,provincial-cities}/`, Mode A `.png` in `pool/magistracies/`) must land in main even though git never carries them - stale or missing renders in main are a bug, not "normal drift". After the locked push succeeds:
     - **Make the clone's renders current** for every map your pushed commits touched. The pytest gate refreshes `.json`+`.svg` but sets `DIAGRAM_SKIP_RENDER=1`, so it never writes a `.png` - run the gens themselves: `ls pool/*/*.gen.py | xargs -P 4 -n 1 python3` from the skill dir regenerates everything (run Mode A gens from their own dir - their output paths are cwd-relative), and `resvg --width 2400 --serif-family 'DejaVu Serif' <plan>.svg <plan>.png` renders a Mode A plan. An engine change that restamps every manifest owes main a FULL render refresh, not just your own maps'.
     - **TIP-GUARD, then copy under the ritual lock, then VERIFY** (from the clone root). `rsync -t` is last-writer-wins - it copies whenever files differ, so a session syncing renders built from an older tip silently clobbers a newer session's correct ones (this happened 2026-07-21: three rounds of a paddy fix never reached the GM because a parallel session's stale render kept overwriting main's). The render-sync invariant is therefore **renders in main come only from main's TIP code**: sync ONLY when your clone is at main's HEAD, and prove the copy landed:

       ```
       flock /gm-assistant/.clones/.ritual.lock sh -c '
         [ "$(git rev-parse HEAD)" = "$(git -C /gm-assistant rev-parse HEAD)" ] || { echo "TIP-GUARD: clone is not at main HEAD - pull, re-gate, re-render first"; exit 1; }
         rsync -rt --include="*/" --include="*.svg" --include="*.png" --exclude="*" .claude/skills/diagram/pool/ /gm-assistant/.claude/skills/diagram/pool/
       ' && md5sum .claude/skills/diagram/pool/<type>/<your-map>.png /gm-assistant/.claude/skills/diagram/pool/<type>/<your-map>.png
       ```

       The trailing md5sum pair (for each map you touched) must MATCH - report the sync from that evidence, never from the rsync exiting 0. And never end a ritual chain with `; echo done` - an unconditional echo after `;` masks a failed `&&` chain (the other half of the 2026-07-21 incident: two rituals' render steps silently never ran while the transcript said "ritual complete"). Chain with `&&` so failure is loud.

       `-rt`, not `-a`: preserving permissions/ownership fails with exit 23 on the rootless-podman bind mount (the data still lands, but the command must exit 0 so failures mean something). Deliberately NO `--delete`: the copy only adds and updates, so a clone that never regenerated some other session's map can never wipe main's render of it. If your commits renamed or deleted a map, `rm` its now-orphaned renders from main by name as part of this step.
     - This is the ONE sanctioned write into main's tree: plain copies of untracked, gitignored files, which no git operation ever touches, so it cannot race another session's push-to-checkout. Running generators or tests against main stays forbidden.
- **Main is the integration point, never a workspace: run NOTHING in main's tree.** No generators, no tests, no writes outside ritual step 5's render sync - any command writing into `/gm-assistant` races with another session's mid-ritual push-to-checkout (2026-07-20: a derived-file regen run in main during another session's push produced a confusingly mixed tree). Gitignored derived renders are BUILT in clones and land in main only via the ritual's render-sync copy (step 5), which keeps main the complete browsing location for every render. Read-only commands against main (`git log`, `git status`, greps) stay fine. **This rule is SELF-ENFORCED**: the diagram skill (`settlement.py` import-time, which every gen/gate/test imports), the webapp rootdir `conftest.py`, and both Makefiles abort with a reminder pointing here when run from main's tree - if you hit that error, re-read this section (reload CLAUDE.md if it has fallen out of context) and rerun from your clone. `GM_ASSISTANT_ALLOW_MAIN=1` is the GM's deliberate override; a session never sets it. (`make serve` in the webapp is exempt - the GM legitimately serves the toolkit from main.)
