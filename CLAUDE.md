...
- **Stop-work ritual - run EVERY time you stop working** (reporting a task done, reaching a milestone, or pausing for GM input):
  ...
  5. **Render sync into main (only when your commits touched diagram pool sources or the engine).** Main's working tree is where the GM browses EVERY pool render in one place, so the gitignored derived renders (Mode B `.svg`+`.png` in `pool/{hamlets,villages,towns,provincial-cities}/`, Mode A `.png` in `pool/magistracies/`) must land in main even though git never carries them - stale or missing renders in main are a bug, not "normal drift". After the locked push succeeds:
     ...
     - **Copy them into main under the ritual lock** (from the clone root):
       ...
       ```
       flock /gm-assistant/.clones/.ritual.lock rsync -a --include='*/' --include='*.svg' --include='*.png' --exclude='*' .claude/skills/diagram/pool/ /gm-assistant/.claude/skills/diagram/pool/
       ```

       Deliberately NO `--delete`: the copy only adds and updates, so a clone that never regenerated some other session's map can never wipe main's render of it. If your commits renamed or deleted a map, `rm` its now-orphaned renders from main by name as part of this step.
