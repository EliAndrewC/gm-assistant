...
| `/name` | Generate Rokugani personal names with meanings in varied formats. Args: `[--refresh] [--avoid A,B] [--bank N] [m\|f] [p] [N]` - shorthand and concatenation (e.g. `pf3`); `--bank N` = N per gender as one distinct set (the backstory name bank). The pool is the ONE source of given names (the `/names` page and the chargen engine read it); used names come from the campaign cache, refreshed via OAuth when stale - no cookie, no loop |
...
| `/chargen` | Make a brand-new NPC end-to-end and upload it to Obsidian Portal: roll a skeleton character with the chargen engine (asking only about genuinely-missing essentials), write a Claude-native backstory (the `/synthesize` method, not Gemini) from the rolled attributes, generate and attach an AI portrait, and create the OP record. Naming is fully scripted (feature 200): `gender=`/`avoid=` constructor args, roster exclusion inside the engine, a scripted name bank for the prose's supporting cast. Public by default; GM-only if the concept says private/hidden. Args: `<free-text character concept>` |
...
| [`docs/session-clones.md`](docs/session-clones.md) | Setting up a clone, hitting a clone/sync hook block, resolving a push conflict, or starting a spec-kit feature while other sessions are running (spec-number claim protocol). |
...
| [`docs/spec-kit-and-reviews.md`](docs/spec-kit-and-reviews.md) | Adding a rule to a review subagent (`backstory-review`, `frontend-review`), or wiring spec-kit hooks. |
...
- **One constitution for the whole repo, deliberately** (GM 2026-08-15): spec-kit's Constitution
  ...
  auto-loading directory CLAUDE.md (the diagram repository's skill CLAUDE.md is the exemplar); SKILL.md stays the
...
- **Record a fix that FAILED, at the point of change.** A cheap wrong lever that measurably did nothing saves the next session from pulling it again - the exemplar is the front-row lane cap's two recorded dead ends in the diagram repository's `hamletgen/homesteads.py`.
...
- **The gate is `make done`, run from `webapp/` inside your clone.** It runs ruff, format-check,
  mypy --strict, the hook-guard suites, pytest and the 100% coverage gate, reports every failure
  together, and stamps the tree so `sync-with-main.sh` can prove a green gate saw the Python being
  pushed. ~15 s on this box. (The diagram repository's `make quick`/`make reference`/`make done`
  ladder and its make-only enforcement hooks moved with it in feature 131; this repository has no
  engine entry points to guard.)
...
- **Iterate on ONE artifact; run the full test bed exactly once, at the end** - but that final sweep is MANDATORY whenever shared code changed. **This binds when BUILDING, not only when debugging** (GM 2026-08-23, constitution VI). A new capability has no artifact "exhibiting the defect", so the whole suite looks like the natural first check; it is not - prove the change on one page, one pool entry, one test file, then sweep. **Every step of a generator feature is TWO steps** - the reference artifact, then the whole pool - and both are tasks (constitution VI). A feature adding a KNOB owes one artifact per knob VALUE.
...
- **Do NOT cut the process steps** (regression-fixture freeze, overlap-registry classification, record-the-why docs, the stop-work procedure). They cost ~2 minutes per feature and are why the regression rate stays near zero. Savings come from turn structure, never from skipping guardrails.

The diagram skill's own dev-loop doctrine moved with it to <https://github.com/EliAndrewC/diagram> (feature 131).

**Improving a review subagent** (`backstory-review`, `frontend-review`, `mention-context-review`): do NOT just apply the fix and write the rule in. The current artifacts contain the motivating defect - that is the failing test. Add the **general, category-level rule only**, run the agent against the unfixed artifact, and only once it FIRES do you fix the artifact and record the specific instance as a validated example. Full procedure, plus the harness gotcha that mid-session edits to `.claude/agents/*.md` do not reach agents launched by type, in [`docs/spec-kit-and-reviews.md`](docs/spec-kit-and-reviews.md).
...
- **Clone name = this session's name, kebab-cased.** Resolve it - do not guess: the hook's `UserPromptSubmit` notice already tells you whether it resolves; by hand, the LAST `{"type":"custom-title"}` record in your transcript (`~/.claude/projects/<cwd-mangled>/<session_id>.jsonl`; `/clear` mints a new id and re-emits the record) is the name, and `~/.claude/sessions/*.json` (grep for your `session_id`, read `.name`) is the fallback - it is rewritten asynchronously, so right after `/clear` it can lag. `gm-assistant` is a FORBIDDEN clone name; an unnamed or auto-derived session has no valid workspace, so ask the GM to `/rename` it before doing repo work.
...
| a spec is reviewed before implementation (XVI) | [`scripts/review-gate.sh`](scripts/review-gate.sh), run by `sync-with-main.sh` at PUSH time |
...
- `/gm-assistant/webapp/` - **L7R Toolkit** (CherryPy + Jinja2). Run with `cherryd --import l7r.app` from this dir (`l7r` itself is a namespace portion with no code in it since feature 119 - the mount is a side effect of `l7r.app`). Routes: `/` landing, `/relics`, `/relics/<slug>`, `/names` (with `?gender=` / `?caste=` filters), `/places`, `/places/<slug>`, `/dreams` (dream-divination framework + example gallery, public `pool/` tier only), `/dreams/<slug>`, `/chargen/*` (legacy chargen mounted as a sub-app). New code under `l7r/`, a **PEP 420 namespace portion** (no `__init__.py`; feature 119 made it one so the diagram engine's `l7r.diagram.*` could share the parent package - that engine is in its own repository since feature 131, so the only remaining reason is not to churn; `tests/test_namespace_portion.py` still guards it). Legacy chargen modules on Principle X grace period.
- `/gm-assistant/scripts/repl.py` - the GM's Python REPL (dice, exact XkY odds, `name()` / `place()` picks off the pools); works from the host (`podman exec` into the container) or inside it. Code and its index in [`webapp/l7r/repl/CLAUDE.md`](webapp/l7r/repl/CLAUDE.md).
- `/gm-assistant/scripts/mention_bot.py` - the Discord mention responder (feature 203): one gateway connection listening, any number of bot tokens speaking, so the bot a player addresses is the bot that answers. Runs on the GM's always-on box, NOT on fly.io - the fly apps scale to zero and holding a socket open would mean paying for a full-time machine. Code and its index in [`webapp/l7r/mention/CLAUDE.md`](webapp/l7r/mention/CLAUDE.md).
- `/gm-assistant/scripts/op_consent_census.py` - consent-first census of other people's L5R campaigns on Obsidian Portal (which owners allow bots); results stay gitignored. `scripts/op_fetch_campaigns.py` then caches and indexes the crawlable campaigns' public pages so a session can summarize and search them. Code and its index in [`webapp/l7r/opcrawl/CLAUDE.md`](webapp/l7r/opcrawl/CLAUDE.md).
...
- `/gm-assistant/.claude/agents/mention-context-review.md` - independent audit of the Discord bots' scripted replies, asking of each whether it makes sense to somebody who does not already know what it refers to (GM 2026-08-31). Run it **one category-group at a time, never in parallel** - the GM's terminal has crashed on fan-out. Its findings and the standard live in [`webapp/l7r/mention/lore/CLAUDE.md`](webapp/l7r/mention/lore/CLAUDE.md).
...
- `/gm-assistant/.claude/skills/name/pool-male.jsonl` + `pool-female.jsonl` - the given-name pool consumed by the `/names` section and the chargen engine
