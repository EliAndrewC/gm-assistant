...
- **Iterate on ONE artifact; run the full test bed exactly once, at the end** - but that final sweep is MANDATORY whenever shared code changed. **This binds when BUILDING, not only when debugging** (GM 2026-08-23, constitution VI). The old wording said "the ONE motivating artifact", which a feature can read as not applying to it - a new capability has no artifact "exhibiting the defect", so the cohort looks like the natural first check. It is not: on feature 126 a 48-map cohort was launched mid-experiment, one seed near-hung, and 30 minutes returned no result, twice. Prove it on **Inashiro** (the canonical hamlet) first; sweep after. A feature adding a KNOB owes one map per knob VALUE - three, not forty-eight.
