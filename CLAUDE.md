...
**Parallel sessions (working alongside another Claude session)**: when the GM says another session is (or will be) concurrently modifying this repo, this session works in an **isolated clone under `.clones/`** rather than sharing the one working tree. A shared tree is what causes the cross-session pain: constant "file modified since read" churn on shared files (`settlement.py`, `pyproject.toml`, ...), a `git status` that mixes both features' uncommitted work, and test-globs (`test_villages` globbing `pool/*.gen.py`, etc.) picking up the other feature's in-flight files. A separate working tree eliminates all of that and converts the inevitable overlap into a clean git merge at reconcile time.

- **`.clones/` is gitignored and lives on the host mount** (`/gm-assistant` is bind-mounted), so a clone there survives a container rebuild. It is the ONLY place this rule relaxes the git-safety convention (below).
- **Precondition - commit a baseline first.** A clone captures only *committed* state, so the GM must commit (and usually push) the current tree before a session clones off it; otherwise uncommitted work is silently missed. This is the single highest-value habit for parallel work.
- **Create the clone (cheap):** `git clone /gm-assistant .clones/<feature>` - a local clone hardlinks the object store, so it is near-instant and does not double the disk. Do all of the feature's work inside `.clones/<feature>`.
- **Git ownership (the relaxation).** Inside `.clones/<feature>` this session performs ALL git actions itself (add / commit / merge / branch) - this is the one place it commits. Main `/gm-assistant` stays the GM's: **the GM does the GitHub push/pull there.** Syncing commits between the clone and main is this session's job (see reconcile), but the public push/pull is the GM's.
- **Reconcile by pulling the clone into main:** from `/gm-assistant`, `git pull .clones/<feature> <branch>`, resolve any conflicts on the shared files, and run the full gate green (`make done` / the skill's `pytest --cov`). **WAIT for the GM's explicit go-ahead before doing this reconcile** - the GM knows whether the other session is mid-edit. Leave main at a resolved-but-unpushed state for the GM to push, unless told otherwise.
- **Gotchas:** keep `pytest`/`ripgrep` scoped to the working dir (git ignores `.clones/`, but those tools do not read `.gitignore`, so a repo-root `pytest` would double-collect the clone's tests). Self-contained features (e.g. `/diagram`) run fully in the clone; a feature that needs `/host-l7r-repo` or `development-secrets.ini` still reaches them by absolute path - only repo-relative paths shift into the clone.
