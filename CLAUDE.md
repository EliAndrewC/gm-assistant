...
- **Iterate on the ONE motivating artifact; run the full test bed exactly once, at the end** - but that final sweep is MANDATORY whenever shared code changed.
...
Package-specific timings and skill-specific lessons live in that skill's dev-loop doc, e.g. [`.claude/skills/diagram/CLAUDE.md`](.claude/skills/diagram/CLAUDE.md), which also carries the DRAW ORDER map and the KEEP-CLEAR CONTRACT.
