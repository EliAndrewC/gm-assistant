...
per Crawl-delay (20 s), never any other owner's campaign pages, and writes the result under the
gitignored webapp/opcache/opcrawl/. About seven minutes. See webapp/l7r/opcrawl/__init__.py.

  ./scripts/op_consent_census.py                 L5R (game_system_id=62)
  ./scripts/op_consent_census.py --game-system-id 62
"""
...
def main() -> int:
    ...
    census = run_census(
        game_system_id=args.game_system_id, progress=lambda s: print(s, flush=True)
    )
