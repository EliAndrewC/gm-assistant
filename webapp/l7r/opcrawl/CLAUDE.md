...
| `pages.py` | `parse_exempt_slugs` - the `data-exempt-cses` list on the pre-human-check gate page, which IS the per-campaign "allow bots" setting: campaign settings shows Privacy as a four-way radio and `internet` ("Public with no human check") is exactly this list, with `public` ("Public with human check"), `friends` and `private` the alternatives (confirmed on the GM's own settings form 2026-08-28; not a paid perk) - and `parse_front_page` for a campaign's name, game system (`system-logo-container` sidebar) and `Last Updated`. |
