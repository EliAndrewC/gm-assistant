...
| `gmrolls.py` | the GM's own `xky` rolls, remembered only while a conversation is open, so `annotate()` can offer one as the opposing side of a contest. Imports nothing from its own package on purpose - `dice.py` writes to it and `rolls/annotate.py` reads from it, and those sit on opposite sides of the import order. |
