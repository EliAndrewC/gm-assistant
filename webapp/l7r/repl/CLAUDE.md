...
| file | holds |
|---|---|
| `__init__.py` | `namespace()` - what the prompt starts with - and the `COMMANDS` banner. **Add a function to `__all__` and it is at the prompt; add it to `COMMANDS` and it is also DOCUMENTED; add its key to `BANNER` and it is also shown AT STARTUP.** Three separate decisions. `help_l7r()` prints the full `COMMANDS` list plus `undocumented()` - every namespace name no row mentions, DERIVED so nothing at the prompt can be invisible. Those are separate: the GM trimmed the banner on 2026-08-29 (*"many of the things have been around for so long that I don't need a reminder of them"*), so `d10`, `xky`, `initiative`, `percent`, `prob`, `place` and the PC constants all still work and simply are not printed. `GENDERS` puts bare `m` and `f` at the prompt so `names(f, 3)` needs no quotes, and `rolls.build_tags()` puts one skill tag per skill in the rules there (`tact`, `sincerity`, ... - feature 207) alongside `vp`. |
| `dice.py` | the rolls and the EXACT XkY distribution (a DP over "how many dice are still >= t"; the docstring has the math). `prob(6, 3)` is a `Dist` that compares and formats as its mean; `prob(6, 3, 20)` is P(>= 20); `prob(6, 3, table=True)` prints a TN table. `prob[6, 3]` / `prob[6, 3, 20]` is dict-style indexing with rerolls on; `prob[False][6, 3]` (and the old `prob[True][6, 3]`) selects reroll explicitly. **`xky` always returns a `DiceTotal` (an `int` subclass)**, so `xky(7, 4) + 8` records the bonus and `_ + 15` afterwards updates that same roll instead of making a new one. `xky(5, 3) - tact` does not subtract: `DiceTotal.__sub__` hands the roll to anything with a `tag_roll` method (duck-typed, because `dice.py` sits below `rolls/` in the import order). Int subclasses cannot have `__slots__`. |
...
| `honor.py` | (PC rank comes off their sheet - `rank=` overrides, and is required for an unregistered PC; Unconventional / Virtue belong to the TARGET and are read from the NPC's OP notes: `Unconventional` always reads low on the first conversation, `Virtue` always high.) `discern_honor(npc, pc, rank=None, upload=True)` - the Discern Honor school knack (`rules/05-school_knacks.md` in the l7r repo). Reads the NPC's true Honor from the `Honor: X.Y` line of the OP GM-only notes, resolves the NPC name with `opsynth.match_character` (whole name tokens only: "Sakura" / "Reiji Sakura" / "Hida Sakura" / "Hida no Reiji Sakura" all work, "Saku" does not; an ambiguous match is an error listing the candidates), keeps a `Discern Honor:` block in those notes - one `- PC (rank N): told X.Y after N conversations` line per PC - and moves the told value `0.rank` closer per conversation until it locks in. The module docstring records the rules-typo decision (`- 5`, not `- 0.5`; the rules text itself was corrected 2026-09-21, with "without rerolling 10s"). **Inside an open conversation with that NPC (feature 212) it returns the value decided at `begin_conversation` - the same one the player's `/discern-honor` gives - and the record advances once however often either is used**; the line then carries a `[c-20260921-7f3a, was 4.5]` marker. With no conversation open against that NPC, one call is one conversation, as before. The conversation half is `rolls/discern.py`. |
| `rolls/` | **its own package** - captures the rolls players post in Discord into an NPC's Obsidian Portal bio, driven by `begin_conversation("Otsuki", "tuesday")` / `end_conversation()`. The GM's recording rules (round down to 5, Etiquette capped at 40, contested keeps both totals and rounds only the margin), a parser for the fifteen real shapes players type, and a read-only Discord client. Index in [`rolls/CLAUDE.md`](rolls/CLAUDE.md). |
| `gmrolls.py` | the GM's own `xky` rolls, buffered whether or not a conversation is open, so `annotate()` can offer one as the opposing side of a contest. Since feature 207 a roll can be TAGGED with its skill (`xky(5, 3) - tact`), marked a mistake, and marked paired. Feature 208 added `penalty` - what a player's oppose roll took off it, kept apart from the GM's own `bonus` - so `total` is `base + bonus - penalty` and `DiceTotal` arithmetic returns the ENTRY's total. Imports nothing from its own package on purpose - `dice.py` writes to it and `rolls/annotate.py` reads from it, and those sit on opposite sides of the import order. |
