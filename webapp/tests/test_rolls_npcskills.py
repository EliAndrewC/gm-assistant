...
class TestTagging:
    def test_the_gm_s_example_records_ring_and_rank(
    ...
    def test_a_second_agreeing_roll_says_nothing_new(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
...
class TestDisagreement:
    ...
    def test_this_roll_spent_a_void_point(
        ...
        assert c.numbers == {'air': 3, 'tact': 2}
        assert total.entry.void_points == 1
        assert not total.entry.mistake
...
class TestCalling:
    def test_from_the_record(self) -> None:
        ...
        assert total.entry.asked == (5, 3)
        assert total.entry.tagged == 'tact'

    def test_a_missing_rank_is_asked_for_within_bounds(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        c = talking(air=3)
    ...
    def test_a_void_point_is_known_about(self) -> None:
        ...
        assert one.entry.asked == (6, 4)
        assert two.entry.asked == (7, 5)
        assert one.entry.void_points == 1
        assert two.entry.void_points == 2
        assert c.numbers == {'air': 3, 'tact': 2}
    ...
    def test_vp_combines_with_a_rank_and_with_a_pool(self) -> None:
        ...
        assert ranked.entry.asked == (6, 4)
        assert pooled is not None
        assert pooled.entry.asked == (7, 5)
        assert c.numbers == {'air': 3, 'tact': 2}
    ...
    def test_rank_zero_does_not_reroll_and_advanced_takes_ten(
        ...
        def die(reroll: bool = True) -> int:
            seen.append(reroll)
            return 5

        monkeypatch.setattr(dice, 'd10', die)
        talking(air=3, tact=0, manipulation=0)
        plain = tact()
        advanced = TAGS['manipulation']()
        assert plain is not None
        assert int(plain) == 15
        assert plain.entry.asked == (3, 3)
        assert advanced is not None
        assert int(advanced) == 5
        assert seen == [False] * 6
...
class TestAutomaticRaises:
    def test_the_called_form_adds_them_and_says_so(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
...
class TestSmallThings:
    def test_vp_reads_like_what_was_typed(self) -> None:
        assert repr(ns.vp) == 'vp'
        assert repr(ns.vp * 2) == 'vp * 2'
    ...
    def test_one_tag_per_rules_skill(self) -> None:
        assert len(TAGS) == 18
        assert TAGS['tact'].ring_name == 'air'
        assert TAGS['history'].advanced
        assert not TAGS['tact'].advanced
