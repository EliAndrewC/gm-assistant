# Imperials and Clan Populations

<!-- SOURCE: GM NOTES - DO NOT MODIFY -->
IMPERIALS:

Each domain has an appointed Imperial samurai (and spouse/children if applicable) acting as an Emerald Magistrate.  They have a staff of around ten yoriki (samurai assistants) with them, who are typically from other clans, with perhaps 1-2 being from one of the Imperial families.  The magistrate and yoriki live in the domain's capital but might visit the domain's other cities on the extremely rare occasion when it is necessary to "travel into the provinces".

When fighting breaks out between neighboring daimyo, the yoriki from nearby domains may be assembled to assist one side or the other.  Having one or two hundred extra troops can change the outcome of a battle, and high-ranking Emerald magistrates are not shy about throwing their weight around.

There are also numerous Imperial legions stationed throughout Rokugan.  We have not yet established specific numbers and locations for all of them, but the ones which we've established to exist are:
- the 1st Imperial Legion guards the Gateway to the Land of the Burning Sands
- the 2nd Imperial Legion guards Beiden Pass at the Spine of the World Mountains
- the 3rd Imperial Legion guards the Kaiu Wall
- most of the remaining Imperial Legions are also stationed along the Kaiu Wall

CLAN POPULATIONS:

Here's a population breakdown in thousands of samurai in each Clan/Family.  Remmeber that samurai are 2% of the population, so a good rule of thumb is that you can multiple these numbers by 50 to get the total number of humans living in the holdings of each of these families (though the actual population density varies by geographic area).

The 7 clans and their populations are listed below, with all numbers being in thousands (e.g. "300" means "300,000" or three hundred-thousand):

CLAN        TOTAL           RULING      FAMILY
            POPULATION      FAMILY      POPULATIONS

Lion        500             Akodo       275 Matsu, 150 Akodo, 50 Ikoma, 25 Kitsu
Crab        300             Hida        175 Hida, 75 Yasuki, 25 Kaiu, 15 Kuni, 10 Hiruma
Crane       275             Doji        100 Doji, 90 Daidoji, 75 Kakita, 10 Asahina
Scorpion    225             Bayushi     125 Bayushi, 80 Shosuro, 15 Soshi, 5 Yogo
Unicorn     225             Shinjo      75 Shinjo, 75 Otaku, 40 Moto, 20 Ide, 15 Iuchi
Dragon      175             Togashi     125 Mirumoto, 20 Togashi, 15 Agasha, 15 Kitsuki
Phoenix     175             Shiba       100 Shiba, 60 Isawa, 15 Asako

In addition to the above numbers, here are the numbers for Imperials and also the Minor clans:

CATEGORY        TOTAL           FAMILY
                POPULATION      POPULATIONS

Imperial        75              45 Seppun, 15 Hantei, 10 Otomo, 5 Miya
Minor Clans     30              30 Mantis, 15 Dragonfly, 5 Fox/Wasp/Sparrow (the "Three-Man Alliance")

Some things to note about these numbers:
- the Hantei family is relatively small despite containing the Emperor himself, and members of the Hantei family usually marry into other families.
- there are a number of minor clans not listed because they are so small, e.g. the Hare clan has fewer than a thousand samurai

Remember that most families are further divided into Houses, each ruled by a local daimyo.  Some smaller families (such as the Yogo) do not have any vassal houses, whereas larger families (such as the Matsu) have dozens of vassal houses.

<!-- END SOURCE -->

## Notes

(AI-generated context, cross-references, and supplementary notes will be added here as the project develops.)
