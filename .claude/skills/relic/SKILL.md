---
name: relic
description: Generate temple relics, cursed items, and supposedly-supernatural objects for the L5R setting, emphasizing the ambiguity between real supernatural effect and superstition
---

# Relics

Generate temple relics, cursed items, famous swords, and supposedly-supernatural objects consistent with the GM's preferred treatment of the supernatural in Rokugan — where supernatural things really do exist, but where superstition is at least as common as genuine supernatural effect, and where it is often difficult to tell which is which.
...
One of the things I would like you to help with in my worldbuilding, in addition to generating vows, is generating supposedly "magic" items or helping craft supposedly supernatural backstories for NPCs.  And one of the most important qualities in that writing is ambiguity.
...
We can build on this notion of ambiguity by reviewing temple relics for the Temple of Bishamon in the city of Toshi Ranbo.  One thing to note is how each relic is incredibly impressive and is known to have many supernatural qualities, but also that there are more mundane explanations (often just "power of suggestion") for each of them!
...
This set of 108 prayer beads is carefully crafted from polished jade, each attuned to a particular guardian spirit revered by the temple reveres. These spirits are a combination of elemental kami, celestial benefactors, major/minor Fortunes, and especially bodisatva. The association of each bead with its patron is well-documented in temple records, though supplicants are not usually told of particulars before they pray, so that they may make use of the prayer beads open to the guidance of any guardian spirit rather than focusing on only 1 single target.
...
Below is the list of 3 relics which were taken.
...
- See `/cosmology/fortunes.md` for Fortune-aligned relics
- See `/cosmology/maho.md` for cursed and bloodspeaker-associated items
- See `/setting/` for setting context (e.g., samurai ranks, clan culture, social structure)
- See `/.claude/skills/temple/SKILL.md` for temple organization (relics belong to temples; this skill assumes that context)
- See `/.claude/skills/vow/SKILL.md` for vows sworn in the presence of relics (the Sword of the First Vow is an example)
- See `/.claude/skills/sword/SKILL.md` for famous swords (which often function as relics in their own right — Yamaoroshi above is an example)
- See `/campaigns/damasu-relic-seekers.md` for the relic seekers investigation profiles
- `/campaigns/temple-relics.md` is a pointer back to this file (campaign-side discoverability index)
