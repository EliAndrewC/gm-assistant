...
**Extra dreams and 10-hunting (rerolls).** A dream die may be rerolled, and a reroll **replaces** the dream - you dream again and keep the new dream. This is how a determined dreamer fishes for a 10. A dreamer may earn **several rerolls in a single night**, drawing on more than one source at once - a void reroll, then a Player Character Point, then Lucky. Reroll sources that apply to dream rolls:

- A **Player Character Point** - reroll any roll, and this may be spent again and again, one reroll for each point ([PC Points rules](https://github.com/EliAndrewC/l7r/blob/master/rules/10-player_character_points.md)).
...
- **A wordless feeling can be the whole sending (a variant form).** Not every meaningful result is an image. A dreamer may keep no dream at all and wake carrying only a strong affect fixed on a target: *you wake filled with <emotion> toward <a person>*, or *with a deep <emotion> about <some face of the matter>* - the road, the granary, the bargain, the man being hunted. This is the one place you **may name the emotion and its object**, because that named affect *is* what the god sent; but the rule still holds one step up - name the feeling and its target, never the god's verdict. *"You wake carrying an unaccountable tenderness toward the old woman at the well"* is in-bounds; *"you wake knowing the god wishes her spared"* is not. (Distinct from the no-dream band's oblique *"nothing, but a feeling,"* which names nothing and means nothing - here the feeling is named, attached, and true.) Because a bare feeling is harder to interrogate than an image and easy to over-trust, it is also a potent **misleading** form: the affect fixes on the wrong target - a fellow sleeper, the foul weather, the hardships of the road, a fear the dreamer already carried - and reads exactly like true intuition. Reach for it in the misleading list as readily as the meaningful one.
...
**A scene page carries only scene-specific content, but the roll table is scene-specific.** The general *procedure* - pick or blindly sub-roll a fragment from the band's list, players never see the tables, the 10 is always significant - is identical for every scene and lives here in the SKILL; do NOT restate that prose on each page. But a scene DOES carry a short **"The roll"** section with a two-column d10 outcome table (`Roll | Result`), because that table realizes the scene's own attunement: which faces are no-dream / meaningless / significant is set by the scene's band counts, and the proportions (e.g. near-ideal = 20% / 20% / 50% no-dream/meaningless/significant, plus 10% significant-and-lucid on the 10) are scene tuning, not general mechanics. Generate the scatter with `randomize_bands.py`, pin the **10 to the significant-and-lucid** result, and add one line noting the non-10 faces are scrambled fresh each night so they cannot be learned. The full band counts / attunement note also belong in the page frontmatter. The roll section should also say whether the odds are **at baseline** (no ill force interested, no personal affliction or party-borne sin, no spiritually perilous matter) or **shifted** by such a factor - and, if baseline, hint at what slightly-different circumstance would tilt them, since a **misleading** band and a lower signal are exactly what those factors add (see the attunement rubric and the public framework page's "When something is listening back").

A page also **describes the design as it currently stands, never the editing history that produced it.** No "we softened this," "flagged for the GM," "originally X," or "conformed to the new standard" in the body or design notes - state what the scene *is*, not what was changed to get there. (Real play *outcomes* are fine as scene metadata; the process of authoring is not.)
