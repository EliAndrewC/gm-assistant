...
- [`hamletgen.md`](hamletgen.md) + [`hamletgen.py`](hamletgen.py) - **EXPERIMENT (2026-08-11), not the live method.** A SCRIPTED Mode B hamlet generator: an eleven-stage pipeline that derives every position from the map's own geometry (the field solved to a real acreage from the household count, the cluster seated on the 背山面水 margin, the pond walked downslope off the drain, the woodland found by scanning open ground) so a whole hamlet is a nine-line spec. Its demo maps are [`pool/experiments/`](pool/experiments/), and `pool/experiments/inashiro` is deliberately the same brief as the hand-authored `pool/hamlets/ikegami` for a head-to-head. **Nothing about the current method changed** - to DRAW a map today, follow this file and `settlements.md` as before. Read `hamletgen.md` before extending, adopting or abandoning it
