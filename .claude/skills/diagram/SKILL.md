...
- [`pyproject.toml`](pyproject.toml) - pytest + coverage config; `fail_under = 100` mechanically enforces full coverage of `check_village/` + `settlement.py`. Run `python3 -m pytest` from the skill dir (needs `pytest` + `pytest-cov`); the full suite must run together or the coverage gate trips
