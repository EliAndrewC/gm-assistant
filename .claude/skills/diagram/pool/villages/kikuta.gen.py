...
_ux, _uy = -_ax, -_ay
_torii = [(round(BEN[0] + _ux * d), round(BEN[1] + _uy * d)) for d in range(58, 58 + 7 * 15, 15)]   # 15px (~30ft) pitch: a DENSE senbon-style avenue (GM 2026-07-22; was 36px/72ft, which read as isolated gates once the torii glyph went true-scale ~8px). Clears the torii_spread_out floor (one arch-span)
