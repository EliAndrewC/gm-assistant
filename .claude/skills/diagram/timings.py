...
the wrong tool for a standing record that has to stay cheap enough to actually re-run.
    ...
    python3 timings.py            # everything (~12 min)
    python3 timings.py --quick    # the inner loops only (~1 min)
    python3 timings.py --dry-run  # list what would run
...
def bench_cohort(count: int, quick: bool) -> Result:
    ...
    secs, ok, _ = sh([PY, "cohort_audit.py", "--count", str(count), "--seed", "1"])
    return Result(
        f"cohort_{count}",
