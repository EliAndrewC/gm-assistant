# test_settlement/ - the engine's unit tests as a package
...
unchanged: same test names, same count, `python3 -m pytest test_settlement/` from the skill dir.
...
`settlement/<module>.py`, the tests to load alongside are `test_settlement/test_<module>.py`.
