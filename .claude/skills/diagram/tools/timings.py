...
the wrong tool for a standing record that has to stay cheap enough to actually re-run.
    ...
    python3 -m tools.timings            # everything (~12 min)
    python3 -m tools.timings --quick    # the inner loops only (~1 min)
    python3 -m tools.timings --dry-run  # list what would run
...
HERE = Path(__file__).resolve().parent.parent  # the skill root; this module lives in tools/
...
def bench_cohort(count: int, quick: bool) -> Result:
    ...
    secs, ok, _ = sh([PY, "-m", "tools.cohort_audit", "--count", str(count), "--seed", "1"])
    return Result(
        f"cohort_{count}",
...
def bench_cache(outdir: str) -> Result:
    ...
    cold, ok_a, _ = sh([PY, "-m", "pipeline.regen", "--no-cache", "pool/hamlets/sawada.gen.py"], NO_RENDER)
    warm, ok_b, out = sh([PY, "-m", "pipeline.regen", "pool/hamlets/sawada.gen.py"], NO_RENDER)
    hit = "CACHED" in out
    return Result(
        "map_regen_sawada",
