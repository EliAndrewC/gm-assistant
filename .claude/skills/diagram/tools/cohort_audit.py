...
incidents on record), so this one only ever quotes what the gate said.
    ...
    python3 -m tools.cohort_audit                 # 12 hamlets from seed 1
    python3 -m tools.cohort_audit --count 24 --seed 1
    python3 -m tools.cohort_audit --only features_do_not_overlap   # just the maps failing one check
    python3 -m tools.cohort_audit --jobs 1        # serial (parallel across processes by default)
"""
