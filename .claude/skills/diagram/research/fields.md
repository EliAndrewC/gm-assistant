...
- *Decision and status.* Finding recorded 2026-08-02; **no map or engine change made yet** - the GM is deciding the target range for the town maps. The direction the evidence supports: drop the accidental town cap entirely; an on-map fan either visibly runs off-frame (a slice), or it is enclosed and must carry at least hamlet-grade acreage (~8+ ac) with a farmstead count its area can feed (~1-2.5 ac/household for the steadings that read as living off it). Do NOT copy the current town gens' comb parameters as a template for new maps in the meantime. (Logged the same day, also pending: the 1 ft/px bund stroke and drain collector render at equal ~1.5 px weight so they interleave where plots sit on the drain; and the farmhouse rings around an envelope can seat a steading across the Imperial Road from its field, inside the urban block - hoshizora's lone south-of-road farmhouse.)
