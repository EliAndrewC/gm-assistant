...
- *The decision it drove.* `settlement.py` `_CROP_MARGIN_FT = 6.0` - bund plus one cut swath (~1.8 m total). The `commons` scatter (all roles) skips every paddy and dry plot padded by this margin, converted at the map's `ftpx`; tall glyphs (scraggly pines ~14*bs px tip reach, woodland crowns ~11.5*bs px radius) additionally stand their own drawn reach back so no ink leans over a crop.
...
- *Deliberately NOT decided here:* a maintained-bank margin. Grass tufts standing right up to a channel's water edge remain - the crop-margin reasoning (banks beside fields were scythed for fodder) plausibly extends to supply-channel banks, but no such clearance has been researched or decided. If the GM wants a cut-bank margin, that is its own rule with its own research entry, not part of this fix.
