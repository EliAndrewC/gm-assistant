...
Keep the entry format: **Grounds:** (which checks or generator methods it supports), then what the research found, the decision it drove, and any disclosed departure. Anchors are stable - rules link to `#slug`, so rename a heading only if you also fix its inbound links. Citations belong here rather than in the rule file: per project policy the *why* is mandatory and explicit sources are optional, so a bare finding is fine and a cited one is better.
