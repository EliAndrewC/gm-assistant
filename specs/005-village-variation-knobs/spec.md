...
- **Focal feature**: an optional, historically-attested village element (crescent pond, ancestral hall, water-mouth complex, mill, market, secondary shrine) drawn from a catalog and placed by existing overlap invariants.
...
- **SC-002**: The automated twin-detector reports zero twinned pairs across the pool (no two same-`down_deg` villages share too many structural axes), AND a human blind review of the regenerated maps (shown without names) yields correct "these are different villages" judgments.
