...
- [ ] T001 Add `ruff` (lint + format) and `mypy` (global `strict = true`) config to `.claude/skills/diagram/pyproject.toml`, keeping the existing pytest + `fail_under = 100` config.
- [ ] T002 [P] Create `.claude/skills/diagram/Makefile` with a `done:` target running `ruff check` + `ruff format --check` + `mypy` + `pytest --cov-fail-under=100`, so all checks run as one command.
- [ ] T003 [P] Document the `make done` gate + how to run it in `.claude/skills/diagram/SKILL.md`.
...
- [ ] T004 Fix the ~10 `ruff` findings (unused vars, lambda-assignments) in `settlement.py`, `check_village.py`, `waterfields.py`; `ruff check` + `ruff format --check` green.
- [ ] T005 Configure the mypy per-module ratchet in `pyproject.toml`: global `strict`, with `settlement.py` + `check_village.py` + `waterfields.py` relaxed via `[[tool.mypy.overrides]]`, so the gate is green while NEW code is strict from day one.
- [ ] T006 Migrate `waterfields.py` to `mypy --strict` (annotate; remove its relax override); pytest stays green. (~99 errors - smallest module first.)
- [ ] T007 Migrate `check_village.py` to `mypy --strict` (annotate; remove its relax override); pytest + coverage stay green. (~1,107 errors - large; move CLI `print` behind the script entrypoint / `logging`.)
- [ ] T008 Migrate `settlement.py` to `mypy --strict` (annotate; remove its relax override); pytest + coverage stay green. (~1,266 errors - largest; a typed manifest structure helps.)

### 2b. Knob machinery core (the shared engine every story uses)

- [ ] T009 [P] Write FAILING unit tests for the seeded roll engine (determinism: same spec+seed -> same draw; independence: different knobs draw separately; two seeds -> different combos) in `test_settlement.py`.
- [ ] T010 Implement the `Knob` registry + `roll(seed, knob_name, context)` (independent, deterministic, seeded) in `settlement.py` (green T009).
- [ ] T011 [P] Write FAILING unit tests for the historical-typing gate (a value invalid for the stated geography is excluded from the roll; a pinned invalid value is rejected/warned) in `test_settlement.py`.
- [ ] T012 Implement the typing-rule gate + the knob-declaration surface (extend `s.meta(...)` to pin/leave-unset knobs; resolution order pinned -> rolled -> default) in `settlement.py` (green T011).
...
- [ ] T013 [P] [US1] Negative fixture + test: the twin-detector FIRES on a deliberately-twinned same-`down_deg` manifest pair, in `test_checks.py`.
- [ ] T014 [P] [US1] Unit tests: headman + primary-shrine positions are DERIVED from `cluster_position` + `lane_skeleton` (not constants), parametrized over skeletons, in `test_settlement.py`.
- [ ] T015 [P] [US1] Unit tests: each Family-A knob's value space + typing rule (parametrized), in `test_settlement.py`.
...
- [ ] T017 [US1] Implement the `lane_skeleton` knob (spine / T / Y / cross / waterside) + DERIVED headman & primary-shrine placement in `settlement.py` (green T014).
...
- [ ] T019 [US1] Implement the `plot_texture` (size + organic/grid regularity) + `grain_drift` knobs in `waterfields.py` (resolves the "uniform 45deg grain" residual).
- [ ] T020 [US1] Implement the focal-feature catalogue placement (crescent pond, ancestral hall, water-mouth complex, mill, market, secondary shrine) in `settlement.py`, each placed by the existing overlap/set-back invariants and recorded in the manifest.
- [ ] T021 [US1] Implement the pool-level twin-detector in `check_village.py` (SC-001 axis discretization + >= 4-of-7 threshold), tuned against the re-varied pool (green T013).
- [ ] T022 [US1] Re-vary `pool/hoshigaoka.gen.py` through the knobs (distinct cluster/lane/water/grain/focal set); `check_village` green + render self-review.
- [ ] T023 [US1] Re-vary `pool/kikuta-village.gen.py` through the knobs so it differs from Hoshigaoka on >= 4 axes; `check_village` green + twin-detector clean + render self-review.
- [ ] T024 [US1] Record the China-first historical grounding for every Family-A knob value + the twin-detector axis set/threshold in `settlements.md` (SC-005).
