...
- [ ] T001 Add the deterministic combine step: one `python3 -m coverage combine --append` line
...
- [ ] T002 Spike test `test_gencache.py::test_a_foreign_parallel_coverage_file_reaches_the_report`:
...
- [ ] T003 [US1] RED: add `test_gencache.py::test_a_dependency_change_invalidates_every_entry`
...
- [ ] T004 [US1] Implement the dependency-state input in `gencache.py`: sorted installed
...
- [ ] T005 [US1] [P] Unit tests to 100% for the new helper including the degradation path;
...
- [ ] T006 [US2] Implement `gencache.gate_obtain(gen)` per the contract: HIT = restore artifacts +
...
- [ ] T007 [US2] Rewire `test_villages.py::_regen_and_gate` to call `gate_obtain`; budget assert
...
- [ ] T008 [US2] Pinning tests in `test_gencache.py` (contract table):
      ...
      - [ ] teeth: reuse-hit
      - [ ] teeth: hit-still-checks
      - [ ] teeth: coverage-less-is-miss
      - [ ] teeth: store/replay round trip
- [ ] T009 [US2] Retire `test_gencache.py::test_the_gate_never_reads_the_cache` (delete, with the
      commit message naming the GM decision it dies by).
- [ ] T010 [US2] Local whole-file runs (`test_gencache.py`, `test_villages.py`), then verify the
...
- [ ] T011 [US3] Pinning test `test_gencache.py::test_gate_bypass_forces_regeneration`
      ...
      fails: - [ ] teeth: bypass
- [ ] T012 [US3] [P] Docs rewrite: `gencache.py` docstring ("WHAT IT DELIBERATELY DOES NOT DO" ->
...
- [ ] T013 [US3] [P] `docs/iteration-loop.md` (repo root): add the GM 2026-08-16 threshold rule -
...
- [ ] T014 [P] `timings.py`: add `warm_gate` benchmark (prime with one cached sweep, then time
