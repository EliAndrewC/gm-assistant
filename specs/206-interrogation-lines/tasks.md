...
- [x] T001 Sync the clone (`git -C /gm-assistant/.clones/discord pull origin main`) and confirm
...
- [x] T002 Create `tests/test_rolls_interrogation.py` with the shared `roll()` / `conversation()`
...
- [x] T003 Add `line: int | None = None` and `grilling: bool = False` to `Roll` in
...
- [x] T004 Add `INTERROGATION = 'interrogation'`, `is_interrogation(roll)`, and
...
- [x] T005 [US1] RED tests in `tests/test_rolls_interrogation.py`: exact total (37 not 35);
...
- [x] T006 [US1] Implement `render_interrogation(rolls, *, bare=False)` in
...
- [x] T007 [US1] Route interrogation rolls through it in `render_lines`
...
- [x] T008 [US2] RED menu tests in `tests/test_rolls_interrogation.py` using a scripted `ask`:
...
- [x] T009 [US2] Extend `Decision` in `l7r/repl/rolls/annotate.py` with `line`, `grilling`,
...
- [x] T010 [US2] Add `_existing_lines(conv, staged)` (conversation rolls overlaid with staged
...
- [x] T011 [US2] Run `pytest -q -n auto tests/test_rolls_annotate.py tests/test_rolls_followup.py`
...
- [x] T012 [US3] RED tests in `tests/test_rolls_interrogation.py`: `y` -> `(grilling)`; blank
      and `n` -> no parenthetical; a roll joining a grilling line inherits `grilling=True` without
      being asked; `yes` / `Y` accepted.
- [x] T013 [US3] Implement the `Grilling? [y/N]` prompt in `_interrogate`
...
- [x] T014 [US4] RED tests in `tests/test_rolls_interrogation.py`: recorded rank -> no prompt
...
- [x] T015 [US4] Add `_optional_number(ask, question) -> int | None` beside `_number` in
...
- [x] T016 [US5] RED tests in `tests/test_rolls_interrogation.py`: `end_conversation()` raises
...
- [x] T017 [US5] Fix whatever T016 exposes in `l7r/repl/rolls/annotate.py` /
...
- [x] T018 Update `l7r/repl/rolls/CLAUDE.md`: add the interrogation line to the "What a written
...
- [x] T019 Sweep: `( cd webapp && make done > <scratch>/gate.log 2>&1 )` and read the tail -
...
- [x] T020 Hand-check per quickstart.md at the REPL (`./scripts/repl.py`, scratch channel):
