...
**Constraints**: nothing runs on AWS unless every condition in FR-007..FR-013 passes; `FULL=1`
cannot run remotely (no TTY - existing guard); the session's credential cannot write a verified
record (R8, needs one admin-key IAM edit); no polling in command text (R5).

**Scale/Scope**: 4 new make targets, 1 changed script (`sync-with-main.sh`), 4 changed targets, 1
new Python package of ~5 modules, 2 buildspecs, 1 Dockerfile, 1 IAM policy edit, CLAUDE.md +
`docs/session-clones.md` + the memory note updated for the new clone `origin`.
...
and this feature does not touch the generator. Two numbers ARE taken because the spec demands them:
local `make done` wall-clock before and after (SC-006, in `timings.md`), and remote `make done`
wall-clock on `xlarge` and `2xlarge` (SC-004, R9, in `timings.md`). The performance-check
INTEGRATION is deferred with the GM's clause quoted (spec, Scope Boundaries).
...
- **XVI. Build What Was Asked (NON-NEGOTIABLE)**: spec FAITHFUL at round 3 after five findings, four
  of them scope the session had added. Two readings remain flagged in the spec's Assumptions
